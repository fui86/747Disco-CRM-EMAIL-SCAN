...
// ✅ FIX CRITICO: Assicura che le variabili esistano SEMPRE
if (!isset($is_googledrive_configured)) {
    $is_googledrive_configured = false;
}
if (!isset($excel_files_list)) {
    $excel_files_list = array();
}

// DEBUG: Log per verifica
error_log('[Excel-Scan-Page] is_googledrive_configured: ' . ($is_googledrive_configured ? 'true' : 'false'));
error_log('[Excel-Scan-Page] excel_files_list count: ' . count($excel_files_list));
...
// Configurazione storage - ✅ USA LA VARIABILE CHE ARRIVA DA class-disco747-admin.php
$storage_type = get_option('disco747_storage_type', 'googledrive');
$storage_configured = $is_googledrive_configured; // ✅ USA QUESTA!
    ...
    <div style="background: white; border-radius: 15px; box-shadow: 0 6px 20px rgba(0,0,0,0.1); margin-top: 20px; overflow: hidden;">
        <div style="background: #f8f9fa; padding: 15px; border-bottom: 1px solid #e9ecef; cursor: pointer;" onclick="toggleDebugSection()">
            <div style="display: flex; align-items: center; justify-content: space-between;">
                ...
                <span id="debug-toggle" style="color: #c28a4d; font-weight: bold;">▼</span>
            </div>
        </div>
...
<script>
// ✅ DEFINISCI window.disco747ExcelScanData PRIMA DI TUTTO
window.disco747ExcelScanData = {
    isConfigured: <?php echo $is_googledrive_configured ? 'true' : 'false'; ?>,
    gdriveAvailable: <?php echo $is_googledrive_configured ? 'true' : 'false'; ?>,
    filesCount: <?php echo count($excel_files_list); ?>,
    ajaxUrl: '<?php echo admin_url('admin-ajax.php'); ?>',
    nonce: '<?php echo wp_create_nonce('disco747_batch_scan'); ?>'
};

// Debug console
console.log('✅ [Excel-Scan-Page] disco747ExcelScanData definito:', window.disco747ExcelScanData);
console.log('✅ [Excel-Scan-Page] isConfigured:', window.disco747ExcelScanData.isConfigured);
console.log('✅ [Excel-Scan-Page] gdriveAvailable:', window.disco747ExcelScanData.gdriveAvailable);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addActivityLog('Google Drive configurato: ' + (window.disco747ExcelScanData.isConfigured ? 'SI' : 'NO'), window.disco747ExcelScanData.isConfigured ? 'success' : 'error');
});
