 ...
 * Versione 11.9.0 - Fix Google Drive check con priorità ai token
 *
...
class Disco747_Admin {
    ...
    public function enqueue_admin_assets($hook_suffix) {
        ...
        try {
            ...
            if (strpos($hook_suffix, 'disco747-scan-excel') !== false) {
                ...
                // ✅ FIX: Usa il nome e nonce corretti per allineamento con la pagina HTML
    ...
    private function render_form_preventivo() {
        ...
        $this->log('Tentativo caricamento form da: ' . $form_file);
        $this->log('File esiste: ' . (file_exists($form_file) ? 'SI' : 'NO'));
        
        if (file_exists($form_file)) {
            $this->log('✅ Caricamento form preventivo');
            require_once $form_file;
        } else {
            $this->log('❌ File form non trovato!', 'error');
            echo '<div class="wrap">';
            echo '<h1>Form Preventivo</h1>';
            echo '<div class="notice notice-error"><p>File form non trovato: ' . esc_html($form_file) . '</p></div>';
            echo '<p><a href="?page=disco747-crm" class="button">← Torna alla Dashboard</a></p>';
            echo '</div>';
        }
    }
    ...
    /**
     * ✅ MODIFICATO v11.9.0: Priorità ai token invece di Client ID/Secret
     * FIX: Il test ha rivelato che i token funzionano ma mancano Client ID/Secret
     */
